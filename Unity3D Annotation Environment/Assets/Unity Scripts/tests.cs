...
public class Placeholder: EditorWindow{
	...
	static void init(){
		...
		// Texture2D img = imgs[0];//.Where(a => a.name == "nice").First() as Texture2D;//Resources.Load <Sprite> ("typ_task_folder/nice");
		// Debug.Log(img.name);
		GameObject li = GameObject.Find("mug");

		Debug.Log(li.GetType().FullName);
